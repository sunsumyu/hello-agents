...
for tool in mcp_tool.get_expanded_tools():
    agent.add_tool(tool)
...
custom_tool = MCPTool(
    ...
    server_command=["python", os.path.join(os.path.dirname(os.path.abspath(__file__)), "my_mcp_server.py")]
)
for tool in custom_tool.get_expanded_tools():
    agent.add_tool(tool)
...
response = agent.run(f"请读取{readme_path}文件，并总结其中的主要内容")
print(response)